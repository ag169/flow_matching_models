...
Uses linear attention blocks + MBConv.
"""
...
class LinAttnDiTBlock(nn.Module):
    """
    ...
    def __init__(
        self, in_dim: int, embed_dim: int, attn_head_dim: int = 64, conv_ksize: int = 3
    ):
        super().__init__()
        ...
        self.norm1 = mu.ChannelRMSNorm(
            in_dim, elementwise_affine=False, transpose_dim=False
        )
        self.norm2 = mu.ChannelRMSNorm(
            in_dim, elementwise_affine=False, transpose_dim=False
        )
        ...
        conv_padding = (conv_ksize - 1) // 2
        c_mid = 4 * in_dim
        self.conv_mlp = nn.Sequential(
            nn.Conv2d(in_dim, c_mid, kernel_size=1, padding=0),
            nn.Conv2d(
                c_mid,
                c_mid,
            ...
            nn.SiLU(),
            nn.Conv2d(c_mid, in_dim, kernel_size=1, padding=0),
        )
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        a1, b1, c1, a2, b2, c2 = torch.chunk(cond_emb, chunks=6, dim=1)
...
class Transformer(nn.Module):
    ...
    def __init__(self, in_dim: int, num_layers: int = 12, embed_dim: int = 128):
        ...
        self.add_pos_embed = mu.LearnedPositionalEmbedding(dim=in_dim)
        self.layers = nn.ModuleList(
            [
                LinAttnDiTBlock(in_dim=in_dim, embed_dim=embed_dim)
